...
class ConstitutionalValidator:
    def validate_file(self, file_path):
        ...
        return Result()
'''
        (repo_path / 'src' / 'constitutional_validator.py').write_text(minimal_validator)
...
'''
    ...
    def test_hook_files_installation(self, temp_git_repo):
        ...
        for hook_name in expected_hooks:
            ...
            if src_hook.exists():
                ...
                assert src_content == dst_content, f"Hook {hook_name} content should match"
    ...
    def test_bash_pre_commit_hook_execution(self, temp_git_repo):
        ...
        try:
            result = subprocess.run(['bash', str(dst_hook)], 
                                  cwd=temp_git_repo, 
                                  capture_output=True, 
                                  text=True,
                                  timeout=30)
            ...
            assert result.returncode in [0, 1], f"Hook returned unexpected code: {result.returncode}"
    ...
    def test_powershell_pre_commit_hook_execution(self, temp_git_repo):
        ...
        try:
            result = subprocess.run(['powershell', '-NoProfile', '-File', str(dst_hook)], 
                                  cwd=temp_git_repo, 
                                  capture_output=True, 
                                  text=True,
                                  timeout=30)
            ...
            # Hook should execute without crashing
            assert result.returncode in [0, 1], f"PowerShell hook returned unexpected code: {result.returncode}"
            
            # Should not crash with syntax errors
            assert "ParseException" not in result.stderr, f"PowerShell hook has syntax errors: {result.stderr}"
    ...
    def test_installation_script_basic_execution(self, temp_git_repo):
        ...
        try:
            ...
            result = subprocess.run(['powershell', '-NoProfile', '-File', str(script_path), '-Help'], 
                                  cwd=temp_git_repo, 
                                  capture_output=True, 
                                  text=True,
                                  timeout=15)
            ...
            assert result.returncode == 0, f"Installation script help failed: {result.stderr}"
            assert "Usage:" in result.stdout or "help" in result.stdout.lower(), "Help should show usage information"
    ...
    def test_hook_performance_requirements(self, temp_git_repo):
        ...
        for hook_name, src_hook in hooks_to_test:
            ...
            try:
                ...
                assert execution_time < 5.0, f"Hook {hook_name} took {execution_time:.2f}s, should be < 5s"
                ...
                end_time = time.time()
                execution_time = end_time - start_time
                assert execution_time < 5.0, f"Hook {hook_name} took {execution_time:.2f}s even when failing"
    ...
    def test_hook_error_handling(self, temp_git_repo):
        ...
        for hook_name in ['pre-commit', 'pre-commit.ps1']:
            ...
            subprocess.run(['git', 'add', 'error_test.py'], cwd=temp_git_repo, check=True)
            ...
            try:
                ...
                assert "Traceback" not in result.stderr, f"Hook {hook_name} crashed with unhandled exception"
                
                # Should provide meaningful error message
                error_output = result.stderr + result.stdout
                meaningful_error = any(keyword in error_output.lower() for keyword in [
                    'validator', 'not found', 'missing', 'error', 'constitutional'
                ])
                assert meaningful_error, f"Hook {hook_name} should provide meaningful error message"
