...
class TestGitHooks:
    ...
    def test_pre_commit_hook_exists(self):
        ...
        hook_files = ["hooks/pre-commit", "hooks/pre-commit.ps1"]
        ...
        for hook_file in hook_files:
            ...
            with open(hook_file, "r", encoding="utf-8") as f:
                ...
                ), f"{hook_file} should reference constitutional validation"

    def test_pre_push_hook_exists(self):
        """Test that pre-push hook files exist and are readable"""
        hook_files = ["hooks/pre-push", "hooks/pre-push.ps1"]
        ...
        for hook_file in hook_files:
            ...
            with open(hook_file, "r", encoding="utf-8") as f:
                ...
                ), f"{hook_file} should reference quality gates"
        ...
        for hook_file in bash_hooks:
            ...
            with open(hook_file, "r", encoding="utf-8") as f:
                ...
                assert first_line.startswith(
                    "#!/bin/bash"
                ), f"{hook_file} should have bash shebang"

            # Run bash syntax check (skip on Windows if bash not available)
            try:
                result = subprocess.run(
                    ["bash", "-n", hook_file], capture_output=True, timeout=10
                )
                assert (
                    result.returncode == 0
                ), f"Bash syntax error in {hook_file}: {result.stderr.decode()}"
        ...
        for hook_file in ps_hooks:
            ...
            with open(hook_file, "r", encoding="utf-8") as f:
                ...
                assert content.startswith(
                    "#"
                ), f"{hook_file} should start with PowerShell comment"
            ...
            try:
                result = subprocess.run(
                    [
                        ...
                        "-NoProfile",
                        "-Command",
                        f'Get-Content "{hook_file}" | Out-Null',
                    ],
                    capture_output=True,
                    timeout=10,
                )
                ...
                if (
                    result.returncode != 0
                    and "powershell" not in result.stderr.decode().lower()
                ):
                    assert (
                        False
                    ), f"PowerShell syntax error in {hook_file}: {result.stderr.decode()}"
    ...
    def test_installation_script_fixed(self):
        ...
        with open(script_file, "rb") as f:
            ...
            for char in problematic_chars:
                assert (
                    char not in content
                ), f"Installation script should not contain Unicode character {char}"

    def test_hook_integration_with_constitutional_validator(
        ...
        assert (
            "src.constitutional_validator" in content
            or "constitutional_validator.py" in content
        )
    ...
    def test_hook_performance_expectations(self):
        ...
        for hook_file in hook_files:
            with open(hook_file, "r", encoding="utf-8") as f:
                ...
                assert (
                    has_error_handling
                ), f"{hook_file} should have error handling for performance"
    ...
    def test_cross_platform_compatibility(self):
        ...
        for hook_file in bash_hooks:
            with open(hook_file, "r", encoding="utf-8") as f:
                ...
                assert (
                    "git rev-parse" in content
                ), f"{hook_file} should use git rev-parse for repo root"
                assert (
                    "/bin/bash" in content
                ), f"{hook_file} should use standard bash path"
        ...
        for hook_file in ps_hooks:
            with open(hook_file, "r", encoding="utf-8") as f:
                ...
                assert (
                    "Join-Path" in content or "Path" in content
                ), f"{hook_file} should use proper path handling"
                assert (
                    "Set-StrictMode" in content
                ), f"{hook_file} should use strict mode"
    ...
    def test_git_reporter_integration(self):
        ...
        for hook_file in hook_files:
            with open(hook_file, "r", encoding="utf-8") as f:
                ...
                assert (
                    "git_reporter" in content
                ), f"{hook_file} should integrate with git reporter"
    ...
    def test_hook_file_permissions(self):
        ...
        for hook_file in bash_hooks:
            ...
            with open(hook_file, "r", encoding="utf-8") as f:
                ...
                assert first_line.startswith(
                    "#!"
                ), f"{hook_file} should start with shebang"
    ...
    def test_installation_script_hook_detection(self):
        ...
        with open(script_file, "r", encoding="utf-8") as f:
            ...
            assert (
                "hooks" in content
            ), "Installation script should reference hooks directory"
            ...
            for hook_ref in hook_references:
                assert (
                    hook_ref in content
                ), f"Installation script should reference {hook_ref}"
...
class TestHookExecutionSimulation:
    ...
    def test_pre_commit_hook_validation_flow(self):
        ...
        if pre_commit_bash.exists():
            content = pre_commit_bash.read_text(encoding="utf-8")
            assert (
                "constitutional_validator" in content
            ), "Hook should reference constitutional validator"
