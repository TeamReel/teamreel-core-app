...
class SecurityScanner:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.fail_on_severity = self.config.get("security", {}).get(
            "fail_on_severity", "medium"
        )
        self.exclude_patterns = self.config.get("security", {}).get(
            "exclude",
            [
                "*/tests/*",
                "*/test/*",
                        ...
                        for pattern in pattern_list:
                            if re.search(pattern, line, re.IGNORECASE):
                                issue = SecurityIssue(
                                    ...
                                    line_number=line_no,
                                    vulnerability_type=self._map_pattern_to_vuln_type(
                                        pattern_type
                                    ),
                                    ...
                                    confidence="medium",
                                    remediation=self._get_pattern_remediation(
                                        pattern_type
                                    ),
                                )
                                issues.append(issue)
    ...
    def _scan_security_patterns_js(self, project_dir: str) -> List[SecurityIssue]:
        ...
        patterns = {
            ...
            "command_injection": [r"exec\(", r"spawn\(", r"child_process"],
            "insecure_random": [r"Math\.random\(", r"crypto\.pseudoRandomBytes"],
        }
                        ...
                        for pattern in pattern_list:
                            if re.search(pattern, line, re.IGNORECASE):
                                issue = SecurityIssue(
                                    ...
                                    description=f"Pattern '{pattern}' matches line: {line.strip()[:100]}",
                                    confidence="medium",
                                    remediation=self._get_pattern_remediation_js(
                                        pattern_type
                                    ),
                                )
                                issues.append(issue)
    ...
    def _create_security_report(
        self, issues: List[SecurityIssue], tools_used: List[str]
    ) -> SecurityReport:
        ...
        medium_issues = len(
            [i for i in issues if i.severity == SecuritySeverity.MEDIUM]
        )
        ...
        for filename, file_issues in files_dict.items():
            file_report = FileSecurityReport(
                ...
                critical_issues=len(
                    [i for i in file_issues if i.severity == SecuritySeverity.CRITICAL]
                ),
                high_issues=len(
                    [i for i in file_issues if i.severity == SecuritySeverity.HIGH]
                ),
                medium_issues=len(
                    [i for i in file_issues if i.severity == SecuritySeverity.MEDIUM]
                ),
                low_issues=len(
                    [i for i in file_issues if i.severity == SecuritySeverity.LOW]
                ),
                issues=file_issues,
            )
    ...
    def _generate_security_suggestions(self, issues: List[SecurityIssue]) -> List[str]:
        ...
        if critical_high:
            suggestions.append(
                f"URGENT: {len(critical_high)} critical/high severity issues require immediate attention"
            )
        ...
        if vuln_counts:
            ...
            for vuln_type, count in top_vulns:
                suggestions.append(
                    f"- {vuln_type.replace('_', ' ').title()}: {count} issues"
                )
    ...
    def format_report(
        self, reports: Dict[str, SecurityReport], format_type: str = "terminal"
    ) -> str:
        ...
        for lang, report in reports.items():
            ...
            output.append(
                f"  Tools used: {', '.join(report.tools_used) if report.tools_used else 'pattern-matching'}"
            )
            ...
            if report.issues:
                ...
                for issue in worst_issues:
                    ...
                    output.append(
                        f"    {severity_icon} {issue.filename}:{issue.line_number} - {issue.title}"
                    )
        ...
        if total_critical + total_high + total_medium + total_low > 0:
            output.append(
                f"Summary: 🔴 {total_critical} critical, 🟠 {total_high} high, 🟡 {total_medium} medium, 🔵 {total_low} low"
            )
...
def main():
    ...
    parser.add_argument(
        ...
        default="medium",
        help="Minimum severity level to fail on",
    )
    ...
    # Configure scanner
    config = {"security": {"fail_on_severity": args.fail_on_severity}}
