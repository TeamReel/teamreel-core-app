...
class ComplexityAnalyzer:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.max_complexity = self.config.get("complexity", {}).get(
            "max_complexity", 10
        )
        self.exclude_patterns = self.config.get("complexity", {}).get(
            "exclude",
            [
                "*/tests/*",
                "*/test/*",
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            "complexity": {
                ...
                "javascript": {"tool": "eslint", "rules": ["complexity"]},
            }
        }
    ...
    def _parse_radon_output(self, radon_json: str) -> ComplexityReport:
        ...
        try:
            ...
            for filename, file_data in data.items():
                ...
                for item in file_data:
                    ...
                    if complexity > self.max_complexity:
                        ...
                        suggestion = self._get_complexity_suggestion(
                ...
                if function_complexities:
                    avg_complexity = sum(function_complexities) / len(
                        function_complexities
                    )
            ...
            suggestions = self._generate_complexity_suggestions(
                all_violations, avg_overall
            )
        ...
        for py_file in python_files:
            try:
                ...
                if file_analysis.function_count > 0:
                    ...
                    total_complexity += (
                        file_analysis.average_complexity * file_analysis.function_count
                    )
    ...
    def analyze_javascript_complexity(self, project_dir: str = ".") -> ComplexityReport:
        ...
        try:
            ...
            cmd = [
                ...
                "eslint",
                "--rule",
                f'complexity: ["error", {self.max_complexity}]',
                "--format",
                "json",
                "**/*.{js,ts,jsx,tsx}",
            ]
    ...
    def _parse_eslint_output(self, eslint_json: str) -> ComplexityReport:
        ...
        try:
                ...
                for message in file_result["messages"]:
                    if message["ruleId"] == "complexity":
                        ...
                        complexity_match = re.search(
                            r"complexity of (\d+)", message["message"]
                        )
                        complexity = (
                            int(complexity_match.group(1))
                            if complexity_match
                            else self.max_complexity + 1
                        )
            ...
            return ComplexityReport(
                ...
                max_complexity=(
                    max([v.complexity for v in violations]) if violations else 0
                ),
                violations_count=len(violations),
                threshold=self.max_complexity,
                violations=violations,
                is_passing=len(violations) == 0,
                remediation_suggestions=self._generate_complexity_suggestions(
                    violations, 0
                ),
            )
    ...
    def _generate_complexity_suggestions(
        self, violations: List[ComplexityIssue], avg_complexity: float
    ) -> List[str]:
        ...
        if high_complexity:
            suggestions.append(
                f"{len(high_complexity)} functions have very high complexity (>20)"
            )
            suggestions.append("Priority: Refactor high-complexity functions first")

        if moderate_complexity:
            suggestions.append(
                f"{len(moderate_complexity)} functions have high complexity (15-20)"
            )
        ...
        worst_violations = sorted(violations, key=lambda x: x.complexity, reverse=True)[
            :3
        ]
        suggestions.append("Most complex functions to address first:")
        for violation in worst_violations:
            suggestions.append(
                f"- {violation.filename}:{violation.line_number} {violation.function_name} (complexity: {violation.complexity})"
            )
    ...
    def format_report(
        self, reports: Dict[str, ComplexityReport], format_type: str = "terminal"
    ) -> str:
        ...
        for lang, report in reports.items():
            ...
            output.append(
                f"  Violations: {report.violations_count} (threshold: ≤{report.threshold})"
            )
            ...
            if report.violations:
                ...
                for violation in sorted(
                    report.violations, key=lambda x: x.complexity, reverse=True
                )[:5]:
                    output.append(
                        f"    • {violation.filename}:{violation.line_number} {violation.function_name} (complexity: {violation.complexity})"
                    )
...
class ComplexityVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        self.function_complexities.append(
            (node.name, self.current_complexity, node.lineno)
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze code complexity and enforce limits"
    )
    parser.add_argument(
        "--max-complexity", type=int, default=10, help="Maximum allowed complexity"
    )
    parser.add_argument(
    ...
    # Configure analyzer
    config = {"complexity": {"max_complexity": args.max_complexity}}
