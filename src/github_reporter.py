...
class GitHubReporter:
    ...
    def __init__(self, repo_name: str = "", base_url: str = ""):
        ...
        self.repo_name = repo_name or os.getenv('GITHUB_REPOSITORY', 'TeamReel/teamreel-core-app')
        self.base_url = base_url or f"https://github.com/{self.repo_name}"
        self.workflow_run_id = os.getenv('GITHUB_RUN_ID', '')
        self.workflow_run_url = f"{self.base_url}/actions/runs/{self.workflow_run_id}" if self.workflow_run_id else ""
    ...
    def generate_compliance_report(self, validation_results: List[ValidationResult]) -> GitHubComplianceReport:
        ...
        avg_score = sum(result.compliance_score for result in validation_results) / total_files if total_files > 0 else 100.0
        ...
        report.status_checks = self._generate_status_checks(validation_results, overall_state)
        ...
        report.workflow_summary = self._generate_workflow_summary(validation_results, report)
        
        return report
    
    def _generate_status_checks(self, validation_results: List[ValidationResult], overall_state: str) -> List[GitHubStatusCheck]:
        """Generate GitHub status checks for constitutional compliance."""
        status_checks = []
        
        # Main constitutional compliance check
        total_violations = sum(len(result.violations) for result in validation_results)
        description = f"Constitutional compliance passed" if overall_state == "success" else f"{total_violations} violations found"
        
        status_checks.append(GitHubStatusCheck(
    ...
    def _generate_pr_comment(self, validation_results: List[ValidationResult], report: GitHubComplianceReport) -> GitHubPRComment:
        ...
        if report.violations_count == 0:
            ...
            for principle, violations in violations_by_principle.items():
                ...
                if len(violations) > 5:
                    body_lines.append(f"*... and {len(violations) - 5} more {principle} violations*")
                    body_lines.append("")
        ...
        body_lines.extend([
            ...
            f"*Run Details: [View Workflow]({self.workflow_run_url})*" if self.workflow_run_url else ""
        ])
    ...
    def _generate_workflow_summary(self, validation_results: List[ValidationResult], report: GitHubComplianceReport) -> str:
        ...
        if report.status_checks:
            ...
            for check in report.status_checks:
                ...
                summary_lines.append(f"- {status_icon} **{check.context}**: {check.description}")
    ...
    def export_status_checks(self, report: GitHubComplianceReport, output_file: str = "github_status_checks.json"):
    ...
    def export_pr_comment(self, report: GitHubComplianceReport, output_file: str = "pr_comment.md"):
    ...
    def export_workflow_summary(self, report: GitHubComplianceReport, output_file: str = "workflow_summary.md"):
...
def main():
    ...
    parser.add_argument("--validation-report", required=True, help="Path to validation results JSON file")
    parser.add_argument("--output-dir", default=".", help="Output directory for reports")
    parser.add_argument("--export-all", action="store_true", help="Export all report formats")
    parser.add_argument("--set-outputs", action="store_true", help="Set GitHub Actions outputs")
    parser.add_argument("--create-summary", action="store_true", help="Create GitHub Actions step summary")
    ...
    try:
        ...
        for file_path, file_data in validation_data.items():
            ...
            from constitutional_validator import ValidationResult, ConstitutionalViolation, SEPrinciple
    ...
    if args.export_all or args.set_outputs:
        status_file = reporter.export_status_checks(report, output_dir / "github_status_checks.json")
    ...
    if args.export_all:
        ...
        summary_file = reporter.export_workflow_summary(report, output_dir / "workflow_summary.md")
