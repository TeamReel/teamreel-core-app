...
class CoverageValidator:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.exclude_patterns = self.config.get('coverage', {}).get('exclude', [
            '*/tests/*',
            '*/test/*',
            '*/__pycache__/*',
            '*/venv/*',
        ...
        self.per_file_threshold = self.config.get('coverage', {}).get('per_file_threshold', 70.0)
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            'coverage': {
                ...
                'javascript': {
                    'tool': 'jest',
                    'args': ['--coverage', '--coverageReporters=json', '--coverageReporters=text']
                }
            }
        }
    ...
    def _parse_python_coverage_json(self, coverage_file: Path) -> CoverageReport:
        ...
        try:
            ...
            suggestions = self._generate_coverage_suggestions(total_coverage, files_below_threshold, files)
    ...
    def _parse_javascript_coverage_json(self, coverage_file: Path) -> CoverageReport:
        ...
        try:
            ...
            for filename, file_data in data.items():
                ...
                covered_statements = sum(1 for count in file_data.get('s', {}).values() if count > 0)
                coverage_pct = (covered_statements / statements * 100) if statements > 0 else 100
                
                missing_lines = [
                    int(line) for line, count in file_data.get('s', {}).items() if count == 0
                ]
            ...
            total_coverage_pct = (total_covered / total_statements * 100) if total_statements > 0 else 100
            suggestions = self._generate_coverage_suggestions(total_coverage_pct, files_below_threshold, files)
    ...
    def _generate_coverage_suggestions(self, total_coverage: float, files_below_threshold: int, files: List[CoverageFile]) -> List[str]:
        ...
        if total_coverage < self.threshold:
            ...
            suggestions.append(f"Coverage is {gap:.1f}% below the {self.threshold}% threshold")
            suggestions.append("Focus on adding tests for uncovered code paths")
        
        if files_below_threshold > 0:
            suggestions.append(f"{files_below_threshold} files have coverage below {self.per_file_threshold}%")
            ...
            for file_obj in low_coverage_files:
                suggestions.append(f"- {file_obj.filename}: {file_obj.coverage:.1f}% coverage ({file_obj.missing}/{file_obj.statements} lines missing)")
    ...
    def format_report(self, reports: Dict[str, CoverageReport], format_type: str = "terminal") -> str:
        ...
        for lang, report in reports.items():
            ...
            output.append(f"\n{lang.upper()} Coverage:")
            output.append(f"  Coverage: {report.total_coverage:.1f}% (threshold: {report.threshold}%)")
            ...
            if report.files_below_threshold > 0:
                output.append(f"  Files below threshold: {report.files_below_threshold}")
...
def main():
    ...
    parser.add_argument("--per-file-threshold", type=float, default=70.0, help="Per-file coverage threshold")
    ...
    parser.add_argument("--fail-under", action="store_true", help="Exit with error if coverage below threshold")
