...
class GitViolationReporter:
    ...
    def _supports_color(self) -> bool:
        ...
        if self.format_type in [GitReportFormat.PLAIN, GitReportFormat.JSON, GitReportFormat.GITHUB_ACTIONS]:
            return False
    ...
    def format_terminal_report(self, violations: List[Any], context: str = "pre-commit") -> str:
        ...
        # Header
        output.append(f"{self.colors['red']}{self.colors['bold']}❌ Constitutional Violations Detected{self.colors['reset']}")
        ...
        for file_path, file_violations in violations_by_file.items():
            ...
            # File header
            output.append(f"{self.colors['blue']}{self.colors['bold']}📁 {file_context.relative_path}{self.colors['reset']}")
            
            # File status indicators
            status_indicators = []
            if file_context.is_staged:
                status_indicators.append(f"{self.colors['green']}staged{self.colors['reset']}")
            if file_context.is_modified:
                status_indicators.append(f"{self.colors['yellow']}modified{self.colors['reset']}")
            
            if status_indicators:
                output.append(f"   {self.colors['cyan']}Status: {' | '.join(status_indicators)}{self.colors['reset']}")
            ...
            for i, violation in enumerate(file_violations, 1):
                ...
                output.append(f"   {severity_color}🚨 Violation #{i}: {rule_id} ({severity}){self.colors['reset']}")
                output.append(f"      {self.colors['white']}Line {line_no} - {se_principle}{self.colors['reset']}")
                
                # Description
                description = getattr(violation, 'description', 'No description available')
                output.append(f"      {self.colors['red']}Issue: {description}{self.colors['reset']}")
                
                # Suggested fix
                suggested_fix = getattr(violation, 'suggested_fix', 'No suggestion available')
                output.append(f"      {self.colors['green']}Fix: {suggested_fix}{self.colors['reset']}")
                
                # Code snippet (if available)
                code_snippet = getattr(violation, 'code_snippet', '').strip()
                if code_snippet:
                    output.append(f"      {self.colors['cyan']}Code:{self.colors['reset']}")
                    for line in code_snippet.split('\n'):
                        output.append(f"        {self.colors['white']}{line}{self.colors['reset']}")
        ...
        high_violations = sum(1 for v in violations if getattr(v, 'severity', '') == 'HIGH')
        medium_violations = sum(1 for v in violations if getattr(v, 'severity', '') == 'MEDIUM')
        low_violations = sum(1 for v in violations if getattr(v, 'severity', '') == 'LOW')
        
        output.append(f"{self.colors['purple']}{self.colors['bold']}📊 Violation Summary{self.colors['reset']}")
        output.append(f"   Total: {total_violations}")
        if high_violations:
            output.append(f"   {self.colors['red']}High: {high_violations}{self.colors['reset']}")
        if medium_violations:
            output.append(f"   {self.colors['yellow']}Medium: {medium_violations}{self.colors['reset']}")
        if low_violations:
            output.append(f"   {self.colors['cyan']}Low: {low_violations}{self.colors['reset']}")
        
        output.append("")
        
        # Next steps
        output.append(f"{self.colors['yellow']}{self.colors['bold']}🔧 Next Steps{self.colors['reset']}")
        ...
        # Documentation links
        output.append(f"{self.colors['blue']}{self.colors['bold']}📖 Documentation{self.colors['reset']}")
    ...
    def format_plain_report(self, violations: List[Any]) -> str:
        ...
        for i, violation in enumerate(violations, 1):
            file_path = getattr(violation, 'file_path', 'unknown')
            rel_path = os.path.relpath(file_path, self.repo_root) if self.repo_root else file_path
            ...
            output.append(f"  Principle: {getattr(violation, 'se_principle', 'Unknown')}")
            output.append(f"  Issue: {getattr(violation, 'description', 'No description')}")
            output.append(f"  Fix: {getattr(violation, 'suggested_fix', 'No suggestion')}")
            output.append("")
    ...
    def format_json_report(self, violations: List[Any]) -> str:
        ...
        report = {
            ...
            'summary': {
                'total_violations': len(violations),
                'high_severity': sum(1 for v in violations if getattr(v, 'severity', '') == 'HIGH'),
                'medium_severity': sum(1 for v in violations if getattr(v, 'severity', '') == 'MEDIUM'),
                'low_severity': sum(1 for v in violations if getattr(v, 'severity', '') == 'LOW'),
    ...
    def format_github_actions_report(self, violations: List[Any]) -> str:
        ...
        for violation in violations:
            ...
            output.append(f"::{annotation_type} file={rel_path},line={line_no},col={col_no}::{message}")
        
        # Summary
        total = len(violations)
        output.append(f"::warning::Constitutional validation found {total} violation(s)")
    ...
    def generate_report(self, violations: List[Any], context: str = "validation") -> str:
    ...
    def write_report_file(self, violations: List[Any], output_path: str, context: str = "validation") -> None:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="TeamReel Git Constitutional Violation Reporter")
    parser.add_argument("--format", choices=["terminal", "plain", "json", "github"], 
